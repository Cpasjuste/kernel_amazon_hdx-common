...
static ssize_t drv260x_read(struct file* filp, char* buff, size_t length, loff_t* offset)
{
	...
	if(pDrv2604data->pReadValue != NULL){
		...
		ret = copy_to_user(buff,pDrv2604data->pReadValue, pDrv2604data->ReadLen);
		if (ret != 0){
...
static ssize_t drv260x_write(struct file* filp, const char* buff, size_t len, loff_t* off)
{
	...
	switch(buff[0])
	{
		...
		case HAPTIC_CMDID_PATTERN_RTP:
		{
			...
			pDrv2604data->vibration_time = (int)((((int)buff[2])<<8) | (int)buff[1]);
			pDrv2604data->silence_time = (int)((((int)buff[4])<<8) | (int)buff[3]);
			pDrv2604data->repeat_times = buff[5];
			strength = buff[6];
		...
		case HAPTIC_CMDID_REG_READ:
		{
			...
			if(pDrv2604data->pReadValue != NULL){
				...
				pDrv2604data->pReadValue = (char *)kzalloc(len-1, GFP_KERNEL);
				if(pDrv2604data->pReadValue == NULL){
					printk("%s, ERROR, pReadValue alloc fail\n",__FUNCTION__);
				}else{
					pDrv2604data->ReadLen = len -1;

					for(i=0;i<(len-1);i++){
						pDrv2604data->pReadValue[i] = drv260x_read_reg(client, buff[i+1]);
					}
				}
			}
...
static int ursa_vib_probe(struct i2c_client *client,
...
static int ursa_vib_remove(struct i2c_client *client)
{
