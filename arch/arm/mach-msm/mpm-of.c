...
#define IDME_BOARD_ID	"/idme/board_id"
#define PRODUCT_BOARD_ID_LEN	16
#define SATURN_BOARD_TYPE	"0010"
#define IDME_BOARD_ID_VERSION_OFFSET	8
#define SATURN_MOD_CLK_BOARD_REV	'6'
static bool board_uses_gps_clk;
...
void __init of_mpm_init(void)
{
	...
	node = of_find_node_by_path(IDME_BOARD_ID);
	WARN_ON(!node);
	board_uses_gps_clk = false;
	if (node) {
		const char *board_id = of_get_property(node, "value", &len);
		if (!strncmp(board_id, SATURN_BOARD_TYPE,
				strlen(SATURN_BOARD_TYPE))
			&& (board_id[IDME_BOARD_ID_VERSION_OFFSET]
				>= SATURN_MOD_CLK_BOARD_REV)) {
			board_uses_gps_clk = true;
		}
	}
}
