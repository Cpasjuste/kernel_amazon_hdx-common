...
#ifdef CONFIG_PM
//for the hook of backlight
extern int lp855x_bl_add_touch_pm_calls(
		int (*new_touch_suspend) (void* dev_context),
		int (*new_touch_resume) (void* dev_context),
		void* new_dev_context
);
extern int lp855x_bl_rm_touch_pm_calls(void);
#endif
...
#define CHIP_A_MASK_REV 0x08A2		//Chip rev
#define CHIP_B_MASK_REV 0x08B1		//Chip rev
...
static int __synaptics_rmi4_suspend(void *dev_context);

static int __synaptics_rmi4_resume(void *dev_context);
#endif
...
static ssize_t synaptics_rmi4_f01_configid_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(buildid, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	 __ATTR(configid, S_IRUGO,
			synaptics_rmi4_f01_configid_show,
			synaptics_rmi4_store_error),
		...
		struct device_attribute *attr, char *buf)
{
	...
	config_id = rmi4_data->rmi4_mod_info.config_id;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	input_report_key(rmi4_data->input_dev, BTN_TOOL_FINGER, touch_count > 0);
#ifndef TYPE_B_PROTOCOL
	if (!touch_count)
		input_mt_sync(rmi4_data->input_dev);
#else
...
static int synaptics_rmi4_read_chip_rev(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	rmi4_data->id_chip = 0;
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f01_query_base_addr + 19,
			buf,
			2);

	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to read chip rev\n",
				__func__);
		return retval;
	}

	revision = buf[0] + (buf[1] << 8);
	dev_info(&rmi4_data->i2c_client->dev,
			"Device chip revision = 0x%04x\n",
			(unsigned short)revision);

	switch (revision) {
	case CHIP_A_MASK_REV:
		rmi4_data->id_chip = 0;
		break;
	case CHIP_B_MASK_REV:
		rmi4_data->id_chip = 1;
		break;
	default:
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Unrecognized device chip revision\n",
				__func__);
		return -EINVAL;
	}
	rmi4_data->id_chip |= ID_VALID;
	return 0;
}
 /**
 ...
 *
 */
static int synaptics_rmi4_query_device_info(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	synaptics_rmi4_read_chip_rev(rmi4_data);
...
static void synaptics_rmi4_input_params(struct synaptics_rmi4_data *rmi4_data)
{
...
#ifdef TYPE_B_PROTOCOL
#ifdef KERNEL_ABOVE_3_7
	//raym: input_mt_init_slots now has a "flags" parameter
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	mutex_lock(&exp_fn_list_mutex);
	if (!rmi4_data->flash_prog_mode && !list_empty(&exp_fn_list)) {
		list_for_each_entry(exp_fhandler, &exp_fn_list, link) {
			if (exp_fhandler->fn_type == RMI_F54) {
				...
				(exp_fhandler->func_init(rmi4_data) == 0) ?
				true : false;
			}
		}
	}
...
static unsigned char TM_SAMPLE1_f1a_button_codes_empty[] = {};

static struct synaptics_dsx_cap_button_map TM_SAMPLE1_cap_button_map_empty = {
	.nbuttons = ARRAY_SIZE(TM_SAMPLE1_f1a_button_codes_empty),
	.map = TM_SAMPLE1_f1a_button_codes_empty,
};
...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_platform_data *pdata)
{
	...
	//raym: Note that the older version of the driver never used what it loaded
	// from the device tree either, but rather used these parameters every time
	pdata->irq_flags = IRQF_TRIGGER_LOW | IRQF_ONESHOT;
	pdata->reset_gpio = of_get_named_gpio(np, "synaptics,reset-gpio", 0);
	pdata->use_id_gpio = of_property_read_bool(np, "synaptics,use-id-gpio");
	pdata->id0_gpio = of_get_named_gpio(np, "synaptics,id0-gpio", 0);
	pdata->id1_gpio = of_get_named_gpio(np, "synaptics,id1-gpio", 0);

	//on Ursa, the regulators are controlled by the display driver
	tmp = 0;
	of_property_read_u32(np, "synaptics,disable-regulator-code", &tmp);
	pdata->regulator_en = tmp ? false : true;

	tmp = 0;
	of_property_read_u32(np, "synaptics,x-flip", &tmp);
	pdata->x_flip = tmp ? true : false;
	tmp = 0;
	of_property_read_u32(np, "synaptics,y-flip", &tmp);
	pdata->y_flip = tmp ? true : false;

	//Include a map of 0 buttons, in case the touch firmware asks for it
	pdata->cap_button_map = &TM_SAMPLE1_cap_button_map_empty;

	dev_info(dev, "flip x %d y %d, use_id %d\n", pdata->x_flip, pdata->y_flip, pdata->use_id_gpio);
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (platform_data->use_id_gpio) {
		if (gpio_is_valid(platform_data->id0_gpio)) {
			retval = gpio_request(platform_data->id0_gpio, "touch_id0");
			if (retval) {
				dev_err(&client->dev,
					"%s: Failed to request touch_id0 GPIO, rc=%d\n",
					__func__, retval);
				goto err_request_id0_gpio;
			}
			retval = gpio_direction_input(platform_data->id0_gpio);
			if (retval) {
				dev_err(&client->dev,
					"%s: Failed to set touch_id0 direction, rc=%d\n",
					__func__, retval);
				goto err_config_id0_gpio;
			}
		}
		if (gpio_is_valid(platform_data->id1_gpio)) {
			retval = gpio_request(platform_data->id1_gpio, "touch_id1");
			if (retval) {
				dev_err(&client->dev,
					"%s: Failed to request touch_id1 GPIO, rc=%d\n",
					__func__, retval);
				goto err_request_id1_gpio;
			}
			retval = gpio_direction_input(platform_data->id1_gpio);
			if (retval) {
				dev_err(&client->dev,
					"%s: Failed to set touch_id1 direction, rc=%d\n",
					__func__, retval);
				goto err_config_id1_gpio;
			}
		}
		rmi4_data->id_gpio = ID_VALID; /* set the GPIO valid bit */
	}
	else
		rmi4_data->id_gpio = 0;
	...
	//so we shouldn't read gpio value immediately after requesting and setting direction
	//the two gpio.
	if (rmi4_data->id_gpio) {
		rmi4_data->id_gpio |= (gpio_get_value(platform_data->id0_gpio) ? 0x1 : 0) |
				      (gpio_get_value(platform_data->id1_gpio) ? 0x2 : 0);

		dev_info(&client->dev, "GPIO based ID=%d\n", rmi4_data->id_gpio & ~ID_VALID);
	}
	else
		dev_info(&client->dev, "GPIO not valid\n");
...
reset_device_again:
	...
	dev_info(&client->dev,"setting irq. GPIO %d, irq %d, type %d\n",
			platform_data->irq_gpio, rmi4_data->irq,
...
#ifdef CONFIG_PM
	//for the hook of backlight
	lp855x_bl_add_touch_pm_calls(__synaptics_rmi4_suspend,
				     __synaptics_rmi4_resume, rmi4_data);
#endif
	return retval;
...
// rails, and then they assume that the power rails are the problem.

err_config_id1_gpio:
	if (gpio_is_valid(platform_data->id1_gpio)) {
		gpio_free(platform_data->id1_gpio);
	}

err_request_id1_gpio:
err_config_id0_gpio:
	if (gpio_is_valid(platform_data->id0_gpio)) {
		gpio_free(platform_data->id0_gpio);
	}

err_request_id0_gpio:
err_config_reset_gpio:
	if (gpio_is_valid(platform_data->reset_gpio)) {
...
err_request_gpio_irq:
	if (platform_data->regulator_en) {
		regulator_disable(rmi4_data->digital_regulator);
		regulator_put(rmi4_data->digital_regulator);
	}

err_digital_regulator:
err_configure_regulator:
	if (platform_data->regulator_en) {
 ...
 */
static int __synaptics_rmi4_remove(struct i2c_client *client)
{
	unsigned char attr_count;
	struct synaptics_rmi4_fn *fhandler;
...
static int __synaptics_rmi4_suspend(void *dev_context)
{
	struct synaptics_rmi4_data *rmi4_data = dev_context;
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	__synaptics_rmi4_suspend(rmi4_data);
...
{
	dev_info(&client->dev, "synaptics shutdown\n");
	__synaptics_rmi4_remove(client);
}
...
static struct i2c_driver synaptics_rmi4_driver = {
	.driver = {
		...
		/* Remove the pm calls, these are taken care of by mdss_dsi_add_touch_pm_calls()
#ifdef CONFIG_PM
