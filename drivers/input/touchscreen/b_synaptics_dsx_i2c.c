...
//Added the touch suspend/resume hooks to the MSM DSI panel driver
// interface, so that the blank/unblank "back door" into the display
// power won't kill the touch driver unexpectedly

//Using the version of these hooks in the panel driver rather than
// in the framebuffer because the framebuffer version gets called
// at the wrong time during the continuous splash init sequence

extern int mdss_dsi_add_touch_pm_calls(
	int (*new_touch_suspend) (void* dev_context),
	int (*new_touch_resume) (void* dev_context),
	void* new_dev_context
);
extern int mdss_dsi_rm_touch_pm_calls(void);
...
#define F01_CONFIG_ID_OFFSET 16
...
int synaptics_rmi4_suspend(void* dev_context);

int synaptics_rmi4_resume(void* dev_context);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned char touch_count = 0; /* number of touch points */
	unsigned char index;
	static unsigned char prev_data15[2] = {0,0};
	unsigned char data15_temp[2] = {0,0};
	unsigned char finger;
	...
	if (extra_data->data15_size) {
		...
		//TODO: Read only the fingers which have changed, rather than
		// finger 0 through finger (highest changed)

		//Currently if you put ten fingers on the screen, then
		// raised fingers 0-8 off the screen, it would keep reading
		// all ten finger reports every frame since the bit for finger 9
		// would still be set
		index = extra_data->data15_size - 1; /* Highest byte */
		finger = (fingers_to_process - 1) % 8; /* Highest bit */

		//------------ < WORKAROUND > -----------------------
		// This is a workaround for PROJB-3811 where the data15 registers
		// won't report on a "finger up" event, only other changes in the
		// finger state
		data15_temp[0] = extra_data->data15_data[0];
		data15_temp[1] = extra_data->data15_data[1];

		if (extra_data->data15_data[0] != prev_data15[0]
			|| extra_data->data15_data[1] != prev_data15[1])
		{
			//Read all the fingers from both the previous report
			// and this one, just in case a finger went up and
			// the firmware decided not to bother us about it
			extra_data->data15_data[0] |= prev_data15[0];
			extra_data->data15_data[1] |= prev_data15[1];
		}

		prev_data15[0] = data15_temp[0];
		prev_data15[1] = data15_temp[1];
		//------------ </ WORKAROUND > -----------------------
	...
	if (!fingers_to_process) {
		//TODO: Verify this behavior with the firmware team
...
static void synaptics_rmi4_empty_exp_fn(struct synaptics_rmi4_data *rmi4_data)
{
	struct synaptics_rmi4_exp_fn *exp_fhandler;
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F34:
				...
				rmi4_data->f34_cmd_base_addr = (&rmi_fd)->cmd_base_addr;
				break;
			}
...
error_exit:
	synaptics_rmi4_empty_exp_fn(rmi4_data);
...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_platform_data *pdata)
{
	...
	pdata->reset_delay_ms = 20;

	//Override the IRQ flags from the device tree, since the DT parser
	// only passes in part of what's entered there
	//TODO: Switch to IRQF_TRIGGER_FALLING
		...
		const struct i2c_device_id *dev_id)
{
	...
	dev_info(&client->dev,
				"%s: B Synaptics RMI4 I2C driver, based on v1.%d\n",
				__func__, (SYNAPTICS_DSX_DRIVER_VERSION&0xFFF) );
	...
	{
		dev_err(&client->dev, "%s: No device tree data found\n",
			 __func__);
		return -EINVAL;
	}
	...
	mdss_dsi_add_touch_pm_calls(synaptics_rmi4_suspend, synaptics_rmi4_resume, rmi4_data);
...
err_parse_dt:
err_platform_data:
	//kfree(platform_data); //This is done automatically by devm_kzalloc
 ...
 *
 * Called by the display driver's BLANK/UNBLANK IOCTL call, to ensure
 * that the in-cell touch controller gets disabled before the display
 *
 ...
 *
 * Called by the display driver's BLANK/UNBLANK IOCTL call, to ensure
 * that the in-cell touch controller gets enabled after the display
 *
