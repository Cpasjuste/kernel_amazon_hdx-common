...
unsigned int ursa_ddic_id_code=0x0;
unsigned int ursa_ddic_ddb=0x0;
static int __init ursa_dispinfo_setup(char *str)
{
	if (str)
		sscanf(str, "0x%x,%x", &ursa_ddic_id_code, &ursa_ddic_ddb);

	//printk("synaptics --%s--, 0x%08x\n", str, ursa_ddic_id_code);
	switch ( ursa_ddic_id_code&0xC0 )
	{
	case 0xC0:
		printk("Synaptics driver just read DDIC ID code from bootloader, supplier is Innolux\n");
		break;
	case 0x80:
		printk("Synaptics driver just read DDIC ID code from bootloader, supplier is JDI\n");
		break;
	default:
		printk("Synaptics driver just read DDIC ID code from bootloader, supplier is unknown or JDI\n");
		break;
	}
	return 1;
}

__setup("dispinfo=", ursa_dispinfo_setup);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		{
		case 0xC0:
			strncpy(fwu->image_name, FW_IMAGE_NAME_INX, MAX_IMAGE_NAME_LEN);
			break;
		case 0x80:
			strncpy(fwu->image_name, FW_IMAGE_NAME_JDI, MAX_IMAGE_NAME_LEN);
			break;
		default:
			dev_dbg(&fwu->rmi4_data->i2c_client->dev,
					"%s: DDIC ID code unknown, skipping firmware update!  (0x%x)\n",
					__func__, ursa_ddic_id_code);
			retval = -EINVAL;
			goto exit;
		}
		dev_dbg(&fwu->rmi4_data->i2c_client->dev,
