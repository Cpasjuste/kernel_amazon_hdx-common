...
static int register_device(struct device_node *dev_node,
		const char *core_id)
{
	struct cyttsp5_device_info info = {0};
	enum cyttsp5_device_type type;
	int rc;
	...
	rc = cyttsp5_register_device(&info);
	if (rc)
		goto fail_free;
...
static void free_core_pdata(struct cyttsp5_core_platform_data *pdata)
{
	int i;
...
{
	struct cyttsp5_core_info info = {0};
	int rc;
	...
	rc = cyttsp5_register_core_device(&info);
	if (rc)
		goto fail_free;
...
EXPORT_SYMBOL_GPL(cyttsp5_devtree_register_devices);

static int __init cyttsp5_devtree_init(void)
{
	return 0;
}
...
module_exit(cyttsp5_devtree_exit);
