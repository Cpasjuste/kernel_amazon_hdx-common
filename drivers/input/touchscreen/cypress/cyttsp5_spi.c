/*
 * cyttsp5_spi.c
 * Cypress TrueTouch(TM) Standard Product V5 SPI Driver module.
...
static int __devinit cyttsp5_spi_probe(struct spi_device *spi)
{
...
static int __devexit cyttsp5_spi_remove(struct spi_device *spi)
{
	struct device *dev = &spi->dev;
...
static struct spi_driver cyttsp5_spi_driver = {
	...
	.remove = __devexit_p(cyttsp5_spi_remove),
