...
#define BOARD_TOUCH_IC_NAME "Saturn_touch"
...
#define BOOT_MODE_ANDROID '1'
#define BOOT_MODE_DIAG    '2'
#define BOOT_MODE_IDX      0
#define BOOT_MODE_IDE_PATH  "/idme/bootmode"

#define BOARD_ID_IDE_PATH  "/idme/board_id"
#define BOARD_ID_STR_SZ 10
#define BOARD_ID_BOARD_TYPE_IDX 8
#define EVT1_CONFIG_VALUE '2'
#define BOARD_ID_WFO_WAN_IDX 9
#define WAN_CONFIG_VALUE '1'
...
static ssize_t fwu_sysfs_read_configid_show(struct device *dev,
...
static struct device_attribute attrs[] = {
	...
	__ATTR(configid, S_IRUGO,
			fwu_sysfs_read_configid_show,
			synaptics_rmi4_store_error),
	__ATTR(readconfig, S_IRUSR,
			synaptics_rmi4_show_error,
	...
	__ATTR(blocksize, S_IRUSR,
			fwu_sysfs_block_size_show,
			synaptics_rmi4_store_error),
	__ATTR(fwblockcount, S_IRUSR,
			fwu_sysfs_firmware_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(configblockcount, S_IRUSR,
			fwu_sysfs_configuration_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(permconfigblockcount, S_IRUSR,
			fwu_sysfs_perm_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(blconfigblockcount, S_IRUSR,
			fwu_sysfs_bl_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(dispconfigblockcount, S_IRUSR,
...
static bool is_EVT1orUnder_device(void)
{
	struct device_node *idme_node;
	int len;
	const char *board_id;

	idme_node = of_find_node_by_path(BOARD_ID_IDE_PATH);
	if (idme_node) {
		board_id = of_get_property(idme_node, "value", &len);
		if ((len >= BOARD_ID_STR_SZ)
		    && (board_id[BOARD_ID_BOARD_TYPE_IDX] > EVT1_CONFIG_VALUE)
		    && (board_id[BOARD_ID_BOARD_TYPE_IDX] <= '9')) {
			return false;
		}
	}
...
static bool is_wfo_device(void)
{
	struct device_node *idme_node;
	int len;
	...
	idme_node = of_find_node_by_path(BOARD_ID_IDE_PATH);
	if (idme_node) {
		const char *board_id = of_get_property(idme_node, "value", &len);
		if ((len >= BOARD_ID_STR_SZ)
		    && (board_id[BOARD_ID_WFO_WAN_IDX] == WAN_CONFIG_VALUE)) {
			return false;
		}
	}
...
static bool is_boot_mode_diag(void)
{
	struct device_node *idme_node;
	int len;

	idme_node = of_find_node_by_path(BOOT_MODE_IDE_PATH);
	if (idme_node) {
		const char *bootmode = of_get_property(idme_node, "value", &len);
		if ((len > 0) && (bootmode[BOOT_MODE_IDX] == BOOT_MODE_DIAG)) {
			return true;
		}
	}

	/** Any mode other than diag is Android mode.. */
	return false;
}
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data, fwu->f34_fd.ctrl_base_addr,
						config_id, sizeof(config_id));
	if (retval < 0) {
		dev_info(rmi4_data->pdev->dev.parent,
			"%s: Couldn't get device config ID, force update FW\n",
			 __func__);
		flash_area = UI_FIRMWARE;
		goto exit;
	}
	dev_info(rmi4_data->pdev->dev.parent,
			"%s: Device config ID = %02x-%02x-%02x-%02x\n",
			__func__,
	...
	if (!fwu->config_data) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Could not get image config ID\n",
				__func__);
		flash_area = NONE;
		goto exit;
	}
	dev_info(rmi4_data->pdev->dev.parent,
			"%s: Image config ID = %02x-%02x-%02x-%02x\n",
			__func__,
			fwu->config_data[0], fwu->config_data[1],
			fwu->config_data[2], fwu->config_data[3]);

	/* Check if image sensor ID match device sersor ID*/
	if ((fwu->config_data[0] & 0x0F) != (rmi4_data->gpio_id & ~ID_VALID)) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image  sensor ID %d don't match device sersor ID %d\n",
				__func__, fwu->config_data[0] & 0x0F, rmi4_data->gpio_id & ~ID_VALID);
		flash_area = NONE;
		goto exit;
	}

	/* Check if in-decive-FW sensor ID match device sersor ID*/
	if ((config_id[0] & 0x0F) != (rmi4_data->gpio_id & ~ID_VALID)) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: In-decive-FW sensor ID %d don't match device sersor ID %d\n",
				__func__, config_id[0] & 0x0F, rmi4_data->gpio_id & ~ID_VALID);
		flash_area = UI_FIRMWARE;
		goto exit;
	}

	/* Check if image FW is newer than in-decive-FW or if different*/
	if (image_fw_id == device_fw_id) {
		/** same firmware, nothing to flash **/
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image FW %d same as in-decive-FW %d, check Config ID..\n",
		...
		/**
		 *    different firmware
		 *       - boot image has a new firmware compared to device - update device firmware
		 *       - boot image has an older firmware compared to device - update device firmware
		 *            This is done to prevent devices from getting bricked and to allow users
		 *            to go back to older builds if required.
		 */
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image FW %d is different than in-device-FW %d, overwriting device FW with one in boot image!\n",
				__func__, image_fw_id, device_fw_id);
		flash_area = UI_FIRMWARE;
		goto exit;
	}

	/* Check if image config newer than in-decive-FW config*/
	if (((fwu->config_data[2] << 8) + fwu->config_data[3]) >
		((config_id[2] << 8) + config_id[3])) {
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image FW config newer than in-device-FW config, overwriting device config area with the one in boot image!\n",
				__func__);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		if (!(fwu->rmi4_data->gpio_id & ID_VALID)) {
			retval = -EINVAL;
			goto exit1;
		}
		strncpy(fwu->image_name, BOARD_TOUCH_IC_NAME, MAX_IMAGE_NAME_LEN);

		/*Saturn EVT1 or under should use wifi fw image.
		  EVT1.1 or upper wifi or wan devie sould use the
		  corresponded fw image.*/
		if (is_EVT1orUnder_device())
			snprintf(fwu->image_name + strlen(BOARD_TOUCH_IC_NAME),
				 MAX_IMAGE_NAME_LEN -
					 strlen(BOARD_TOUCH_IC_NAME),
				 ".%s.%d.img", "wifi",
				 fwu->rmi4_data->gpio_id & ~ID_VALID);
		else
			snprintf(fwu->image_name + strlen(BOARD_TOUCH_IC_NAME),
				 MAX_IMAGE_NAME_LEN -
					 strlen(BOARD_TOUCH_IC_NAME),
				 ".%s.%d.img",
				 is_wfo_device() ? "wifi" : "wan",
				 fwu->rmi4_data->gpio_id & ~ID_VALID);
		dev_info(rmi4_data->pdev->dev.parent,
...
exit:
	...
	dev_info(rmi4_data->pdev->dev.parent, "Device is in %s mode\n",
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (is_boot_mode_diag() == false) {
		...
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Diagnostics mode, Skipping Touch FW update!! \n", __func__);
#endif
