...
struct synaptics_rmi4_data {
	...
	unsigned int gpio_id;	/* the id values as read from gpio, ID_VALID means it valid */
	unsigned int chip_id;	/* the chip rev id read from hw chip, ID_VALID means it valid */
	int irq;
	int sensor_max_x;
	int sensor_max_y;
...
struct synaptics_rmi4_exp_fn {
	...
	void (*attn)(struct synaptics_rmi4_data *rmi4_data,
			unsigned char intr_mask);
	void (*active)(struct synaptics_rmi4_data *rmi4_data,
			unsigned char intr_mask,
		        int *is_active);
};
