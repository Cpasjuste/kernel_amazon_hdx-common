...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_board_data *pdata)
{
	...
	if (IS_ERR(vdd_ana)) {
		...
		dev_err(dev, "pma8084_l18 setup failed \n");
		return -1;
	}

	err = regulator_enable(vdd_ana);
	if (err) {
		dev_err(dev, "regulator_enable() err: %d\n", err);
		return -1;
	}
	tmp = 0;
	...
	dev_info(dev, "%s: flip x %d y %d, use_id %d\n", __func__, pdata->x_flip, pdata->y_flip, pdata->use_id_gpio);
	return 0;
}
#else
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_board_data *pdata)
