...
static ssize_t synaptics_rmi4_flip_x_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_flip_x_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_flip_y_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_flip_y_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct synaptics_rmi4_exp_fn active_pen_handlers;

static struct device_attribute attrs[] = {
#ifdef CONFIG_HAS_EARLYSUSPEND
	__ATTR(full_pm_cycle, (S_IRUSR | S_IWUSR),
	...
	__ATTR(productinfo, S_IRUSR,
			synaptics_rmi4_f01_productinfo_show,
			synaptics_rmi4_store_error),
	__ATTR(buildid, S_IRUGO,
			synaptics_rmi4_f01_buildid_show,
			synaptics_rmi4_store_error),
	__ATTR(flashprog, S_IRUSR,
			synaptics_rmi4_f01_flashprog_show,
			synaptics_rmi4_store_error),
	__ATTR(0dbutton, (S_IRUSR | S_IWUSR),
			synaptics_rmi4_0dbutton_show,
			synaptics_rmi4_0dbutton_store),
	__ATTR(suspend, S_IWUSR,
			synaptics_rmi4_show_error,
			synaptics_rmi4_suspend_store),
	__ATTR(flip_x, (S_IRUSR | S_IWUSR),
			synaptics_rmi4_flip_x_show,
			synaptics_rmi4_flip_x_store),
	 __ATTR(flip_y, (S_IRUSR | S_IWUSR),
			synaptics_rmi4_flip_y_show,
			synaptics_rmi4_flip_y_store),
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
			rmi4_data->hw_if->board_data->x_flip);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rmi4_data->hw_if->board_data->x_flip = input ? true : false;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
			rmi4_data->hw_if->board_data->y_flip);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rmi4_data->hw_if->board_data->y_flip = input ? true : false;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	trace_tracing_mark_write(1, "TOUCH_PUSH", 1);
	...
	trace_tracing_mark_write(1, "TOUCH_PUSH", 0);
		...
		unsigned char intr_mask)
{
	...
	is_pen_active = 0;
	if (rmi4_data->ignore_finger && (active_pen_handlers.active != NULL))
		active_pen_handlers.active(rmi4_data, intr_mask, &is_pen_active);

	if (is_pen_active) {
		dev_dbg(rmi4_data->pdev->dev.parent, "Active pen event pending.. not sending touch input_sync()");
		return 0;
	}
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	trace_tracing_mark_write(1, "TOUCH_IRQ", 1);
...
exit:
	trace_tracing_mark_write(1, "TOUCH_IRQ", 0);
	return IRQ_HANDLED;
}
...
/**
 * synaptics_rmi4_read_chip_rev()
 *
 * Called by synaptics_rmi4_probe().
 *
 * Get the touch chip revision
 */
static int synaptics_rmi4_read_chip_rev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->chip_id = buf[0] + (buf[1] << 8);
	dev_info(rmi4_data->pdev->dev.parent,
			"Device chip revision = 0x%04x\n",
			rmi4_data->chip_id);

	rmi4_data->chip_id |= ID_VALID;
	return 0;
}
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_gpio_setup(bdata->irq_gpio,
			true, 0, 0, "synaptics_irq");
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
	...
	if (bdata->power_gpio >= 0) {
		retval = synaptics_rmi4_gpio_setup(bdata->power_gpio,
				true, 1, !power_on, "synaptics_pwr");
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	if (bdata->reset_gpio >= 0) {
		retval = synaptics_rmi4_gpio_setup(bdata->reset_gpio,
				true, 1, !reset_on, "synaptics_rst");
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
	...
	if (gpio_is_valid(bdata->id0_gpio) && gpio_is_valid(bdata->id1_gpio)) {
		retval = synaptics_rmi4_gpio_setup(bdata->id0_gpio,
				true, 0, 0, "synaptics_id0");
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to configure id0 %d GPIO\n",
				__func__, bdata->id0_gpio);
			rmi4_data->gpio_id = 0;
		}
		else {
			retval = synaptics_rmi4_gpio_setup(bdata->id1_gpio,
					true, 0, 0, "synaptics_id1");
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to configure id1 %d GPIO\n",
					__func__, bdata->id1_gpio);
				synaptics_rmi4_gpio_setup(bdata->id0_gpio,
					false, 0, 0, NULL);
				rmi4_data->gpio_id = 0;
			}
			else{
		...
		bool insert)
{
	...
	if (insert) {
		...
		if (exp_fn->fn_type == RMI_ACTIVE_PEN) {
		    memset(&active_pen_handlers, 0, sizeof(struct synaptics_rmi4_exp_fn));
		    active_pen_handlers = *exp_fn;
		    pr_info("%s: Inserting active_pen new_function\n", __func__);
		}
	} else if (!list_empty(&exp_data.list)) {
		if (exp_fn->fn_type == RMI_ACTIVE_PEN) {
		    memset(&active_pen_handlers, 0, sizeof(struct synaptics_rmi4_exp_fn));
		    pr_info("%s: Removing active_pen new_function\n", __func__);
		}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	/* add the regulator code here if needed */
	/*
	...
	*/
	...
	if (rmi4_data->gpio_id & ID_VALID) {
		rmi4_data->gpio_id |= gpio_get_value(bdata->id0_gpio) ? 0x1 : 0;
		rmi4_data->gpio_id |= gpio_get_value(bdata->id1_gpio) ? 0x2 : 0;
		dev_info(&pdev->dev, "%s: GPIO ID %d\n",
				__func__, rmi4_data->gpio_id & ~ID_VALID);
	}
...
static int __synaptics_rmi4_suspend(void *dev_context)
{
	...
	dev_err(rmi4_data->pdev->dev.parent, "ENTER %s\n", __FUNCTION__);
	if (rmi4_data->stay_awake) {
		dev_err(rmi4_data->pdev->dev.parent, "%s: stay_awake set, not suspending\n", __FUNCTION__);
		return 0;
	}
	...
	*/
	...
	dev_err(rmi4_data->pdev->dev.parent, "EXIT %s\n", __FUNCTION__);
...
static int __synaptics_rmi4_resume(void *dev_context)
{
	...
	if (rmi4_data->stay_awake) {
		dev_err(rmi4_data->pdev->dev.parent, "%s: stay_awake set, nothing to resume\n", __FUNCTION__);
		return 0;
	}
