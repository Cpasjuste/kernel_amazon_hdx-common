...
static ssize_t rmidev_sysfs_palm_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t rmidev_sysfs_palm_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(pid, S_IRUSR | S_IWUSR,
			rmidev_sysfs_pid_show,
			rmidev_sysfs_pid_store),
	__ATTR(term, S_IWUSR,
			synaptics_rmi4_show_error,
			rmidev_sysfs_term_store),
	__ATTR(intr_mask, S_IRUSR | S_IWUSR,
			rmidev_sysfs_intr_mask_show,
			rmidev_sysfs_intr_mask_store),
	__ATTR(palm, S_IRUSR | S_IWUSR,
			rmidev_sysfs_palm_show,
			rmidev_sysfs_palm_store),
};
