...
/* fw image name:
   when use touch id: FW_IMAGE_NAME.id.SUFFIX
   when not use id:   FW_IMAGE_NAME.SUFFIX
*/
#if defined(CONFIG_ARCH_MSM8974_APOLLO)
#define FW_IMAGE_NAME   "Synaptics"
#define FW_IMAGE_SUFFIX ".apollo.img"
#elif defined(CONFIG_ARCH_MSM8974_THOR)
#define FW_IMAGE_NAME   "Synaptics"
#define FW_IMAGE_SUFFIX ".thor.img"
#else
#define FW_IMAGE_NAME   "PR1063486-s7301_00000000"
#define FW_IMAGE_SUFFIX ".img"
#endif
#define MAX_FIRMWARE_ID_LEN   10
#define MAX_FIRMWARE_NAME_LEN 35
...
static int fwu_reset_device(void)
{
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			"fwu->f01_fd.cmd_base_addr = 0x%04x\n",
...
static int fwu_wait_for_idle(int timeout_ms)
{
	...
	do {
		...
		usleep_range(SLEEP_TIME_US, SLEEP_TIME_US);
...
static enum flash_area fwu_go_nogo(void)
{
	...
	/* device firmware id */
	deviceFirmwareID = extract_uint(fwu->rmi4_data->rmi4_mod_info.build_id);
	deviceFirmwareID &= 0xFFFFFF;

	dev_info(&i2c_client->dev,
			"Device firmware id %d.\n", deviceFirmwareID);

	/* device config id */
	devConfigID = fwu->rmi4_data->rmi4_mod_info.config_id;

	dev_info(&i2c_client->dev,
		"Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		devConfigID[0], devConfigID[1], devConfigID[2], devConfigID[3]);
	...
	//Check if img sensor id match HW GPIO id
	id_image = fwu->config_data[0] & MASK_4BIT;
	id_gpio = fwu->rmi4_data->id_gpio & ~ID_VALID;
	if ((fwu->rmi4_data->id_gpio & ID_VALID) && (id_gpio != id_image)) {
		dev_warn(&i2c_client->dev,
			"[img config]\t%s image sensor id (%d) not eqaul to gpio sensor id (%d).\n",
			__func__,
			id_image,
			id_gpio);
		flash_area = NONE;
	...
	//Check if img chip rev id match HW chip rev id
	id_image = (fwu->config_data[0] >> 4) & MASK_4BIT;
	id_chip = fwu->rmi4_data->id_chip & ~ID_VALID;
	if ((fwu->rmi4_data->id_chip & ID_VALID) && (id_chip != id_image)) {
		dev_warn(&i2c_client->dev,
			"[img config]\t%s image chip id (%d) not eqaul to HW chip id (%d).\n",
			__func__,
			id_image,
			id_chip);
		flash_area = NONE;
	...
	//Check if device sensor id not match gpio id, force upgrade.
	id_device = devConfigID[0] & MASK_4BIT;
	if ( (fwu->rmi4_data->id_gpio & ID_VALID) && (id_gpio != id_device)) {
		dev_info(&i2c_client->dev,
			"[dev config]\t%s device sensor id (%d) not eqaul to gpio sensor id (%d). Force upgrade\n",
			 __func__,
			 id_device,
			id_gpio);
		flash_area = UI_FIRMWARE;
	...
	//Check if device chip rev id not match HW chip rev id, force upgrade.
	id_device = (devConfigID[0] >> 4) & MASK_4BIT;
	if ( (fwu->rmi4_data->id_chip & ID_VALID) && (id_chip != id_device)) {
		dev_info(&i2c_client->dev,
			"[dev config]\t%s device chip id (%d) not eqaul to HW chip id (%d). Force upgrade\n",
			 __func__,
			 id_device,
			 id_chip);
		flash_area = UI_FIRMWARE;
	...
	//img release number is largger than device release number
	if (fwu->config_data[1]> devConfigID[1]) {
		dev_info(&i2c_client->dev,
			"[dev config]%s device fw id (%d) less than img fw id (%d). Upgrade fw\n",
			 __func__,
			 devConfigID[1],
			fwu->config_data[1]);
		flash_area = UI_FIRMWARE;
	...
	//config id 3rd and 4th
	if ( ((fwu->config_data[2] << 8) + fwu->config_data[3]) >
	     ((devConfigID[2] << 8) + devConfigID[3]) ) {
		dev_info(&i2c_client->dev,
			"[dev config]%s device config id (%d) less than img cnfig id (%d). Upgrade config\n",
			 __func__,
			 ((devConfigID[2] << 8) + devConfigID[3]),
...
/* Galvatron boards either has board_id not set, or not same as 'c' or 'd' */
static bool is_board_galvatron(void)
{
...
static int fwu_start_reflash(void)
{
	...
	char *fwFile = kzalloc(sizeof(MAX_FIRMWARE_NAME_LEN), GFP_KERNEL);
	...
	if (fwu->ext_data_source)
		...
		if (strlen(FW_IMAGE_NAME) + strlen(FW_IMAGE_SUFFIX) + 3 > MAX_FIRMWARE_NAME_LEN) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: fw image too long, name %s suffix %s\n",
					__func__, FW_IMAGE_NAME, FW_IMAGE_SUFFIX);
			retval = -EINVAL;
			goto exit;
		}

		strcpy(fwFile, FW_IMAGE_NAME);
		if (is_board_galvatron())
			strcat(fwFile, ".g");
		else if (fwu->rmi4_data->id_gpio & ID_VALID)
			retval = sprintf(fwFile + strlen(FW_IMAGE_NAME), ".%1d", (fwu->rmi4_data->id_gpio & ~ID_VALID));

		//If Chip hung in bootloader mode, could not get the chip rev through i2c, default as chip rev B
		if ( fwu->rmi4_data->flash_prog_mode && !(fwu->rmi4_data->id_chip & ID_VALID) ){
			dev_info(&fwu->rmi4_data->i2c_client->dev,
				"%s: Device is in bootloader mode, chip id not valid, use default rev B\n",
				__func__);
			fwu->rmi4_data->id_chip = 1 + ID_VALID;
		}

		if (fwu->rmi4_data->id_chip & ID_VALID){
			sprintf(fwFile + strlen(FW_IMAGE_NAME) + retval, ".%c", 'A' + (fwu->rmi4_data->id_chip & ~ID_VALID));
			retval = 0;
		}

		strcat(fwFile, FW_IMAGE_SUFFIX);
		dev_info(&fwu->rmi4_data->i2c_client->dev,
				"%s: Requesting firmware image %s\n",
				__func__, fwFile);

		retval = request_firmware(&fw_entry, fwFile,
		...
		if (retval != 0) {
			...
			/* try the generic fw (e.g. without the id) */
			if ( (fwu->rmi4_data->id_gpio & ID_VALID) && (fwu->rmi4_data->id_chip & ID_VALID)) {
				strcpy(fwFile, FW_IMAGE_NAME);
				strcat(fwFile, FW_IMAGE_SUFFIX);
				dev_info(&fwu->rmi4_data->i2c_client->dev,
						"%s: Requesting generic firmware image %s\n",
						__func__, fwFile);
				...
				dev_warn(&fwu->rmi4_data->i2c_client->dev,
					"%s: id_gpio %d or/and id_chip %c not valid\n",
					__func__,
					fwu->rmi4_data->id_gpio & ~ID_VALID,
					fwu->rmi4_data->id_chip & ~ID_VALID);
