...
static struct delayed_work temp_notify_work;
#endif
...
#if defined(CONFIG_ARCH_MSM8974_THOR) || defined(CONFIG_ARCH_MSM8974_APOLLO)
static void __ref temp_notify(struct work_struct *work)
{
	struct tsens_device tsens_dev;
	long temp = 0;
	...
	int ret = 0;
	int i = 0;
	unsigned int cpu_sensors[] = {5,6,7,8};

	for (i=0; i < ARRAY_SIZE(cpu_sensors); i++) {
		tsens_dev.sensor_num = cpu_sensors[i];
		...
		if (ret) {
			...
			if (temp > 100) {
				pr_info("%s: TSENS(%d) temperature is %d\n",
					KBUILD_MODNAME, tsens_dev.sensor_num, (int)temp);
				ret = tsens_get_warm_temp_threshold(&tsens_dev, &temp_threshold);
				if (ret)
					pr_debug("%s: Unable to read TSENS sensor threshold %d\n",
						KBUILD_MODNAME, tsens_dev.sensor_num);
				else
					pr_info("%s: TSENS(%d) threshold temperature is %d\n",
						KBUILD_MODNAME, tsens_dev.sensor_num, (int)temp_threshold);
			}
		}
	}

	schedule_delayed_work(&temp_notify_work, 4 *HZ);
}
#endif
...
#if defined(CONFIG_ARCH_MSM8974_THOR) || defined(CONFIG_ARCH_MSM8974_APOLLO)
	INIT_DELAYED_WORK(&temp_notify_work, temp_notify);
	schedule_delayed_work(&temp_notify_work, 0);
#endif
