				  ...
				  const void *data)
{
	...
	CDBG("Set s_ctrl to driver data");
	/* set driver data here so that we can access it in the sysfs routine */
	ret= dev_set_drvdata(&pdev->dev,(void*)s_ctrl);
	if(ret)
		pr_err("dev_set_drvdata FAILED");
	else
		lab126_sensor_sysfs_file_create (&pdev->dev);
