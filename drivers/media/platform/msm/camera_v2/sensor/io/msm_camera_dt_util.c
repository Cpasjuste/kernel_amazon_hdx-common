	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CLK:
			...
			CDBG("%s:%d CLK enable %ld", __func__, __LINE__, power_setting->config_val);
		...
		case SENSOR_GPIO:
			...
			if (!ctrl->gpio_conf->gpio_num_info->valid
				[power_setting->seq_val])
				continue;
			CDBG("%s:%d gpio set[%ld] val  to GPIO[%d]\n", __func__, __LINE__,
		...
		case SENSOR_VREG:
			...
			CDBG("%s:%d VREG enable %d", __func__, __LINE__, power_setting->seq_val);
