...
struct bq27x41_info {
	...
	int suspend_drain;
	int suspend_drain_high;
	int suspend_realtime;
	/* ACOS_MOD_END {metrics_log} */
};

//static int bq27x41_battery_fac = 0;
...
static void battery_handle_work(struct work_struct *work)
{
	...
	if (info->battery_voltage <= BQ27X41_VOLT_CRIT_THRESHOLD) {
		...
		snprintf(buf, sizeof(buf),
			"bq27x41:def:critical_shutdown=1;CT;1:NR");
			...
			union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SUSPEND_DRAIN:
		val->intval = info->suspend_drain;
		break;
	case POWER_SUPPLY_PROP_SUSPEND_DRAIN_HIGH:
		val->intval = info->suspend_drain_high;
		break;
	case POWER_SUPPLY_PROP_SUSPEND_REALTIME:
		val->intval = info->suspend_realtime;
		break;
	/* ACOS_MOD_END {metrics_log} */
	default:
		return -EINVAL;
