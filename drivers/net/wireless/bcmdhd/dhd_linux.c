 ...
 *
 * $Id: dhd_linux.c 494974 2014-08-05 02:51:16Z $
 */
...
uint dhd_max_rx_ms = 20;
module_param(dhd_max_rx_ms, uint, S_IWUSR | S_IRUGO);
MODULE_PARM_DESC(dhd_max_rx_ms, "DHD max RX time in ms. 0: disabled");

uint dhd_max_rx_pkt;
module_param(dhd_max_rx_pkt, uint, S_IWUSR | S_IRUGO);
MODULE_PARM_DESC(dhd_max_rx_pkt, "DHD max RX packet no. 0: disabled");
...
#endif
#ifdef SUPPORT_MULTIPLE_REVISION
#define BCM4354_FW_PATH "/vendor/firmware/fw_bcmdhd_4354.bin"
#define BCM4354_APFW_PATH "/vendor/firmware/fw_bcmdhd_apsta_4354.bin"
#define BCM4354_NV_PATH "/etc/wifi/bcmdhd_4354.cal"
#define BCM4356_FW_PATH "/vendor/firmware/fw_bcmdhd_4356.bin"
#define BCM4356_APFW_PATH "/vendor/firmware/fw_bcmdhd_apsta_4356.bin"
#define BCM4356_NV_PATH_WFO "/etc/wifi/bcmdhd_4356_WFO.cal"
#define BCM4356_NV_PATH_WAN "/etc/wifi/bcmdhd_4356_WAN.cal"
extern bool board_has_wan(void);
#endif
...
dhd_dpc(ulong data)
{
	...
	if (dhd->pub.busstate != DHD_BUS_DOWN) {
		...
		if (isresched && (dhd_dpc_prio < 0)) {
			wake_up_process(pub->rx_thread);
			isresched = 0;
		}
		else
			DHD_OS_WAKE_UNLOCK(&dhd->pub);
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (nvram_path[0] != '\0')
		nv = nvram_path;
#ifdef SUPPORT_MULTIPLE_REVISION
      /* Override the module parameter to support SATURN EVT1.0 as
       * well as EVT2.0
       *
      */
      switch(get_chip_id()) {
      ...
      case BCM4354_D11AC_ID:
	      if (strstr(fw, "_apsta") != NULL)
	              fw = BCM4354_APFW_PATH;
	      else
		      fw = BCM4354_FW_PATH;
              nv = BCM4354_NV_PATH;
              break;
      ...
      case BCM4356_D11AC_ID:
		if (strstr(fw, "_apsta") != NULL)
			fw = BCM4356_APFW_PATH;
		else
			fw = BCM4356_FW_PATH;

		if (!board_has_wan())
			nv = BCM4356_NV_PATH_WFO;
                else
			nv = BCM4356_NV_PATH_WAN;
              break;
      default:
              DHD_ERROR(("chip not supported \n"));
              break;
      }
#endif
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#ifdef LAB126_FEATURE_SUPPORT
#ifdef PREFER_ASSOCIATION_5G
	uint32 band = WLC_BAND_5G;
	uint assoc_pref = WLC_BAND_AUTO;
#endif /* PREFER_ASSOCIATION_5G */
#endif /* LAB126_FEATURE_SUPPORT */
    ...
    dhd_wl_ioctl_cmd(dhd, WLC_SET_ASSOC_PREFER, (char *)&assoc_pref, sizeof(band), TRUE, 0);
    DHD_ERROR(("%s: Association preference set to auto \n", __FUNCTION__));
	...
	/* Enable HT Tx STBC and HT Rx STBC capability*/
	{
		uint8 stbc_buf = 1;

		bcm_mkiovar("stbc_rx", &stbc_buf, 1, iovbuf, sizeof(iovbuf));
		ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
					sizeof(iovbuf), TRUE, 0);
		if (ret < 0)
			DHD_ERROR(("%s Set stbc_rx Enable Failed %d\n",
					__func__, ret));

		bcm_mkiovar("stbc_tx", &stbc_buf, 1, iovbuf, sizeof(iovbuf));
		ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
					sizeof(iovbuf), TRUE, 0);
		if (ret < 0)
			DHD_ERROR(("%s Set stbc_tx Enable Failed %d\n",
					__func__, ret));
	}
#endif /* !defined(FORCED_TO_2G_ONLY) && !defined(FORCED_TO_5G_ONLY) && defined(PREFER_ASSOCIATION_5G) */
#endif /* LAB126_FEATURE_SUPPORT */
...
#ifdef LAB126_FEATURE_SUPPORT
	setbit(eventmask, WLC_E_WAKE_EVENT);
#endif /* LAB126_FEATURE_SUPPORT */
#ifndef WL_CFG80211
...
#ifdef LAB126_FEATURE_SUPPORT
int dhd_get_iface_addr(struct net_device *dev, u32 *ipv4)
{
	dhd_info_t *dhd;
	...
	ret = dhd_arp_get_arp_hostip_table(&dhd->pub, ipv4_buf, sizeof(ipv4_buf), idx);
	 if (ret) {
                DHD_ERROR(("%s failed\n", __FUNCTION__));
                return -1;
        }
...
int dhd_set_dfs_skip(struct net_device *dev, int dfskip)
{
       dhd_info_t *dhd;
       dhd_pub_t *dhd_pub;
       dhd = *(dhd_info_t **)netdev_priv(dev);
       if (!dhd){
               DHD_ERROR(("%s net dev priv is NULL\n", __FUNCTION__));
               return -1;
       }
       dhd_pub = &dhd->pub;
       dhd_pub->dfs_skip = dfskip;
       return 0; 
}
#endif /* LAB126_FEATURE_SUPPORT */
