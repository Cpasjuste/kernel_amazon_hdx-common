 ...
 *
 * $Id: dhd_pcie_linux.c 495226 2014-08-06 02:21:54Z $
 */
...
dhdpcie_pci_remove(struct pci_dev *pdev)
{
	...
	kthread_stop(bus->dhd->rx_thread);
...
int dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		bus->dhd->rx_thread = kthread_run(dhd_rx_thread,
						  bus->dhd->info, "bcmdhd_rx");
		if (!bus->dhd->rx_thread) {
			DHD_ERROR(("%s thread creation failed\n", __FUNCTION__));
			break;
		}
