 ...
 *
 * $Id: dhd_msgbuf.c 495226 2014-08-06 02:21:54Z $
 */
#include <typedefs.h>
#include <osl.h>
...
static int dhd_prot_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring,
				    uint8 *buf, uint16 len, uint16 *pktnum);
static int dhd_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring,
			       uint8 *buf, uint16 len, uint16 *pktnum);
...
{
	dhd_prot_t *prot = dhd->prot;
	unsigned long timeout = jiffies + msecs_to_jiffies(dhd_max_rx_ms);
	uint16 pktnum = 0;
	...
	while (TRUE) {
		...
		if (dhd_prot_process_msgtype(dhd, prot->d2hring_rx_cpln, src_addr,
			src_len, &pktnum) != BCME_OK) {
			prot_upd_read_idx(dhd, prot->d2hring_rx_cpln);
			DHD_ERROR(("%s: Error at  process rxpl msgbuf of len %d\n",
				__FUNCTION__, src_len));
		}

		/* Yield CPU to other threads if RX has taken maximum of time.
		 * rx_thread is to reschedule the tasklet for fetching remaining
		 * RX packets from ring buffer.
		 */
		if (dhd_max_rx_ms && time_after_eq(jiffies, timeout))
			return TRUE;

		/* If over maximum RX packets, yield CPU to other threads.
		 * dhd_dpc is to reschedule the tasklet for fetching remaining
		 * RX packets from ring buffer.
		 */
		if (dhd_max_rx_pkt && (pktnum >= (uint16)dhd_max_rx_pkt))
			return TRUE;
	}
...
static int BCMFASTPATH
dhd_prot_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring,
			 uint8 *buf, uint16 len, uint16 *pktnum)
{
	dhd_prot_t *prot = dhd->prot;
	...
	while (len > 0) {
		...
		if (dhd_process_msgtype(dhd, ring, buf, (uint16)cur_dma_len,
					pktnum) != BCME_OK) {
...
static int BCMFASTPATH
dhd_process_msgtype(dhd_pub_t *dhd, msgbuf_ring_t *ring,
		    uint8 *buf, uint16 len, uint16 *pktnum)
{
	uint16 pktlen = len;
	uint16 msglen;
...
prot_ring_attach(dhd_prot_t * prot, char* name, uint16 max_item, uint16 len_item, uint16 ringid)
{
	...
	/* Initialize marker for D2H rings */
	if (ringid >= BCMPCIE_D2H_MSGRING_CONTROL_COMPLETE &&
		ringid <= BCMPCIE_COMMON_MSGRING_MAX_ID) {
