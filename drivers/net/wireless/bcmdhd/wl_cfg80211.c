 ...
 *
 * $Id: wl_cfg80211.c 494808 2014-08-04 13:29:53Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static s32 wl_notify_wake_event(struct bcm_cfg80211 *cfg,
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data);
...
#else
			/* SKIP DFS channels for Secondary interface as well as primary as configured by App layer*/
			if ((cfg->escan_info.ndev != bcmcfg_to_prmry_ndev(cfg) || (dfs_skip_sta )) &&
				(request->channels[i]->flags &
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, 0))
				(IEEE80211_CHAN_RADAR | IEEE80211_CHAN_PASSIVE_SCAN)))
#else
...
#else
                wl_scan_prep(&params->params, request, dhd_pub->dfs_skip);
#endif
...
#ifdef MFP
			if (pval == AES_ENABLED) {
				if (((wpa2_ie = bcm_parse_tlvs((u8 *)sme->ie, 
					sme->ie_len,
					DOT11_MNG_RSN_ID)) != NULL) &&
					(wl_cfg80211_get_rsn_capa(wpa2_ie, 
						rsn_cap) == 0)) {

					if (rsn_cap[0] & RSN_CAP_MFPC) {
						/* MFP Capability advertised 
						 * by supplicant. Check 
						 * whether MFP is 
...
#ifdef LAB126_FEATURE_SUPPORT
	WL_ERR(("Connecting with" MACDBG " channel (%d), len (%d)\n\n",
...
#ifdef LAB126_FEATURE_SUPPORT
static s32
wl_notify_wake_event(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
						const wl_event_msg_t *e, void *data)
{

	printf("\r\n <<<<< RCVD %d %x >>>> \r\n",ntoh32(e->event_type),ntoh32(e->reason));
	return 0;
}
#endif /* LAB126_FEATURE_SUPPORT  */
...
#ifdef LAB126_FEATURE_SUPPORT
	cfg->evt_handler[WLC_E_WAKE_EVENT] = wl_notify_wake_event;
#endif /* LAB126_FEATURE_SUPPORT */
}
