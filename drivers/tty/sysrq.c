...
static void sysrq_handle_watchdog(int key)
{
	spinlock_t lock;
	unsigned long flags;
	spin_lock_init(&lock);
...
static struct sysrq_key_op sysrq_watchdog_op = {
	.handler	= sysrq_handle_watchdog,
	.help_msg	= "triggger-watch-dog(Y)",
	.action_msg	= "Trigger watch dog",
	.enable_mask	= SYSRQ_ENABLE_DUMP,
};
