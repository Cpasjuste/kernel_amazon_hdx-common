							...
							struct timespec *ts)
{
	...
	case ANDROID_ALARM_GET_WAKEUP_MASK:
		/* Until SATURN-4009 is resolved, we can't rely on WEV_RTC to
		 * be set when we check for resume event in alarm_triggered.
		 *
		 * This means, we will not set the WAKEUP_TRIGGER flag for
		 * AlarmService to read. So, don't return the real mask yet,
		 * instead return the RTC_WAKEUP | ELAPSED_WAKEUP_UP mask and
		 * fix this when the original JIRA is resolved.
		 *
		 * Fix Me : ssp - change to ANDROID_ALARM_WAKEUP_TRIGGER_MASK
		 */
		rv = ANDROID_ALARM_WAKEUP_MASK;
		break;
	default:
		rv = -EINVAL;
	}
							...
							ktime_t now)
{
	...
	/* Check if we were just woken up from suspend broke the device out of
	 * suspend
	 */
	if (WEV_RTC == pm_get_resume_ev(&last_resume_time)) {
		if (ktime_compare(last_resume_time, last_wakeup_time) > 0) {
			devalrm->type |=
				ANDROID_ALARM_WAKEUP_TRIGGER_MASK;
			last_wakeup_time = last_resume_time;
		}
	}
