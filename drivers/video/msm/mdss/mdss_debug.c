...
int psr_user_enabled = 0; /* defaults to OFF */

static int mdss_psr_open(struct inode *inode, struct file *file)
{
	/* non-seekable */
	file->f_mode &= ~(FMODE_LSEEK | FMODE_PREAD | FMODE_PWRITE);
	file->private_data = inode->i_private;
...
static int mdss_psr_release(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t mdss_psr_read(struct file *file,
			char __user *buff, size_t count, loff_t *ppos)
{
	char buf[10];
	int len;
...
static ssize_t mdss_psr_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	u32 data, cnt;
	char buf[10];
	...
	cnt = sscanf(buf, "%x", &data);

	if (cnt < 1)
		return -EFAULT;

	psr_user_enabled = data ? 1 : 0;

	pr_info("Set PSR to %d\n", psr_user_enabled);
	pr_info("Please touch screen to initiate frame update for PSR setting to take effect\n");

	return cnt;
}

static const struct file_operations mdss_psr_fops = {
	.open = mdss_psr_open,
	.release = mdss_psr_release,
	.read = mdss_psr_read,
	.write = mdss_psr_write,
};
...
int mdss_debugfs_init(struct mdss_data_type *mdata)
{
	...
	debugfs_create_file("psr",  0644, mdd->root, mdata, &mdss_psr_fops);
