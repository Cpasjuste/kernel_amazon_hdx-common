...
DEFINE_LED_TRIGGER(bl_led_trigger);
	...
	defined(CONFIG_ARCH_APQ8084_LOKI) || defined(CONFIG_ARCH_APQ8084_SATURN)
...
int mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (gpio_is_valid(ctrl_pdata->disp_lcd_en_gpio)) {
		...
		if (rc) {
			pr_err("request panel disp_lcd_en_gpio failed,rc=%d\n",
								rc);
...
int loki_saturn_mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	lcd_en_gpio = ctrl_pdata->disp_lcd_en_gpio;

	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		pr_debug("%s:%d, reset disp_en not configured\n",
			   __func__, __LINE__);
	}
...
int apollo_mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	int rc = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
	...
	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		pr_info("%s:%d, reset disp_en not configured\n",
				__func__, __LINE__);
	}
	...
	if (enable) {
		...
		if (gpio_is_valid(ctrl_pdata->mbist_gpio))
			gpio_direction_output(ctrl_pdata->mbist_gpio, 0);
		udelay(200);
		...
		if (gpio_is_valid(ctrl_pdata->disp_lcd_en_gpio))
			gpio_direction_output(ctrl_pdata->disp_lcd_en_gpio, 1);
		msleep(15);
...
int thor_mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	int rc = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
	...
	if (enable) {
		...
		/* below was rst low first then disp_en low, sequence changed for KK */
		gpio_set_value((ctrl_pdata->rst_gpio), 0);
		gpio_free(ctrl_pdata->rst_gpio);
...
int __mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	int rc = 0, i;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	/* Adding platform/panel specific reset functions
	 * For new targets, new checks and functions should be added.
	 */
#if defined(CONFIG_ARCH_APQ8084_LOKI) || defined(CONFIG_ARCH_APQ8084_SATURN)
	rc = loki_saturn_mdss_dsi_panel_reset(pdata, enable);
#elif defined(CONFIG_ARCH_MSM8974_APOLLO)
	rc = apollo_mdss_dsi_panel_reset(pdata, enable);
#elif defined(CONFIG_ARCH_MSM8974_THOR)
	rc = thor_mdss_dsi_panel_reset(pdata, enable);
#else
	rc = __mdss_dsi_panel_reset(pdata, enable);
#endif
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (data) {
		if (!strncmp(data, "bl_ctrl_wled", 12)) {
			if (led_trigger_is_registered(bl_led_trigger)) {
