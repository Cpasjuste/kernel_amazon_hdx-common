...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	pr_debug("%s: nothing to do on ursa\n", __func__);
		return rc;
#endif
	...
	if (enable) {
		...
		wmb();
#endif
		/* removed the else case since it's covered below */
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	pr_debug("%s: ctrl=%p ndx=%d, on_cmds.cmd_cnt=%d\n", __func__, ctrl, ctrl->ndx, ctrl->on_cmds.cmd_cnt);
