...
static int mdss_dsi_regulator_init(struct platform_device *pdev)
{
	...
	static int regulators_already_initialized = 0;
	...
	if (regulators_already_initialized)
	{
		pr_err("%s: attempting to re-initialize display regulators\n", __func__);
		return -EINVAL;
	}
	...
	if (ctrl_pdata->power_data.num_vreg > 0) {
		pr_err("%s: Error!  Attempted to use msm_dss_config_vreg on ursa.\n"
			"msm_dss_config_vreg is NOT supported on ursa, because it does not support\n"
			"hardware vendors' power-on sequences\n",
				__func__);
		return -EINVAL;
	}
	...
	ret = regulator_set_voltage(dsi_drv->vdd_vreg, 3150000, 3150000);
	if (ret) {
	...
	if (IS_ERR(dsi_drv->vdd_io_vreg)) {
		pr_err("could not get vdd_io power regulator, rc = %ld\n",
	...
	dsi_drv->vdd_io_panel_vreg = devm_regulator_get(&pdev->dev, "vddio_panel");
	if (IS_ERR(dsi_drv->vdd_io_panel_vreg)) {
		pr_err("%s: could not get vddio_panel reg, rc=%ld\n",
			__func__, PTR_ERR(dsi_drv->vdd_io_panel_vreg));
		return PTR_ERR(dsi_drv->vdd_io_panel_vreg);
	}
	...
	regulators_already_initialized = 1;
	return 0;
}
#else
static int mdss_dsi_regulator_init(struct platform_device *pdev)
...
#if defined(CONFIG_MACH_URSA)
/* This timing sequence is very precise and _MUST NOT_ be modified.
 *
 * tA, tB, etc are timings specified by our hardware vendor see
 * "P1_Display_Verification_Document_Rev8.0" for more information.
 *
 * DO NOT MODIFY WITHOUT CONSULTING THE URSA BSP TEAM
 */
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (ret) {
			pr_err("%s: Failed to enable regulator vdd_io_vreg.\n", __func__);
			return ret;
		}

		//vdd_io_panel_vreg == IVDDI == LVS2 on P1
		ret = regulator_enable((ctrl_pdata->shared_pdata).vdd_io_panel_vreg);
		if (ret) {
			pr_err("%s: Failed to enable regulator vdd_io_panel_vreg.\n", __func__);
			return ret;
		}
		wmb();

		//L2, power for the DSI lines
		ret = regulator_enable((ctrl_pdata->shared_pdata).vdda_vreg);
		if (ret) {
			pr_err("%s: Failed to enable 1.2V DSI phy regulator.\n", __func__);
			return ret;
		}

		//tC is the rise time of those regulators (IVDDI & co)
		gpio_set_value((ctrl_pdata->touch_rst_gpio), 1);
		mdelay(2); //tH
		wmb();

		if (pdata->panel_info.panel_power_on == 0) {
			pr_info("%s:%d, enabling gpios\n", __func__, __LINE__);
			if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
				pr_info("%s:%d, display enable line not configured\n",
					   __func__, __LINE__);
				return -EINVAL;
			}
			...
			gpio_set_value((ctrl_pdata->disp_en_gpio), 1); //+- 5.4V rails
			//Note: +- rails turn on at the same time,
			// but spec calls for 2ms between
			// and 5ms after
			mdelay(2); //tE + tF + tG
			mdelay(1); //tN
			wmb();
		...
		ret = regulator_disable(
			(ctrl_pdata->shared_pdata).vdd_io_panel_vreg);
		if (ret) {
			pr_err("%s: Failed to disable regulator.\n",
				__func__);
			return ret;
		...
		gpio_set_value((ctrl_pdata->touch_rst_gpio), 0);
		mdelay(15); //tV + tW
...
#ifdef CONFIG_FB_MSM_MIPI_JDI_WQXGA_PANEL
...
#ifndef CONFIG_ARCH_APQ8084_MOCHA_COMMON
...
#if defined(CONFIG_MACH_URSA)
	ctrl_pdata->touch_rst_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,platform-touch-rst-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->touch_rst_gpio)) {
		pr_err("%s:%d, touch reset gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->touch_rst_gpio, "disp_tch_rst_n");
		if (rc) {
			pr_err("request touch reset gpio failed, rc=%d\n",
				rc);
			return -ENODEV;
		}
		//Note: If this BSP were going to be reused later, this should use the
		// cont_splash_enabled flag rather than "1" here
		rc = gpio_direction_output(ctrl_pdata->touch_rst_gpio, 1);
		if (rc) {
			pr_err("set_direction for touch reset gpio failed, rc=%d\n",
				rc);
			return -ENODEV;
		}
	}
