...
static struct completion link_ready;

void hdmi_hdcp_auth_complete(void)
{
	DEV_DBG("%s: HDCP authentication continue\n", __func__);
	if (!completion_done(&link_ready)) {
		DEV_INFO("%s: Complete pending authentication\n",
		 __func__);
		complete_all(&link_ready);
	}
}
EXPORT_SYMBOL(hdmi_hdcp_auth_complete);
...
static void hdmi_hdcp_auth_work(struct work_struct *work)
{
	...
	/* wait for slimport link to be stable before authenticate */
	INIT_COMPLETION(link_ready);
	DEV_INFO("%s: Wait for HDMI link to be stable\n", __func__);
	timeout_count = wait_for_completion_timeout(
		&link_ready, (slimport_support_enabled() ? HZ * 2 : 0));

	DEV_INFO("%s: Link stable: %s\n", __func__,
