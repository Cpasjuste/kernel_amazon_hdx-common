				      ...
				      enum led_brightness value)
{
	...
	pr_debug("request to set brightness: %d\n", bl_lvl);
		...
		struct mdp_display_commit *disp_commit)
{
	...
	if (mfd->mdp.precommit_fnc) {
		ret = mfd->mdp.precommit_fnc(mfd);
		if (IS_ERR_VALUE(ret)) {
			pr_err("pre commit failed on fb%d\n", mfd->index);
			return ret;
		}
	}
...
static void mdss_fb_var_to_panelinfo(struct fb_var_screeninfo *var,
	struct mdss_panel_info *pinfo, bool split_screen)
{
        if (split_screen) {
                // Don't allow frambuffer-level ioctl to change geometry
                // of split screen configuration - the interface is
                // not parameterized appropriately for a dual screen.
                return;
        }
	pinfo->xres = var->xres;
	pinfo->yres = var->yres;
			     ...
			     struct fb_info *info)
{
	...
	if (mfd->panel_info) {
		...
		mdss_fb_var_to_panelinfo(var, &panel_info, mfd->split_display);
...
static int mdss_fb_set_par(struct fb_info *info)
{
	...
	if (mfd->panel_reconfig || (mfd->fb_imgType != old_imgType)) {
		...
		mdss_fb_var_to_panelinfo(var, mfd->panel_info, mfd->split_display);
