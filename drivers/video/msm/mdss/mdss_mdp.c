...
#else
int psr_user_enabled; /* OFF */
#endif /* CONFIG_DEBUG_FS */

static ssize_t psr_read(struct device *dev,
...
static ssize_t psr_write(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);

	sscanf(buf, "%d", &psr_user_enabled);
	return ret;
}

static DEVICE_ATTR(psr, S_IRUGO | S_IWUSR, psr_read, psr_write);
...
static struct attribute_group psr_group = {
	.attrs = psr_attrs,
};

static int mdss_psr_create_sysfs(struct platform_device *pdev)
{
	int rc = 0;
	struct mdss_data_type *mdata = platform_get_drvdata(pdev);
	rc = sysfs_create_group(&mdata->pdev->dev.kobj, &psr_group);
	if (rc)
		pr_err("%s: sysfs group creation failed, rc=%d\n", __func__,
			rc);
	return rc;
}
static void mdss_psr_remove_sysfs(struct platform_device *pdev)
{
	struct mdss_data_type *mdata = platform_get_drvdata(pdev);
	sysfs_remove_group(&mdata->pdev->dev.kobj, &psr_group);
}
