...
static int bluetooth_set_power(void *data, bool blocked)
{
	...
	printk(KERN_INFO "bluetooth_set_power set blocked=%d", blocked);
	if (!blocked) {
#ifdef CONFIG_BCM4335BT
		...
		printk(KERN_DEBUG "Bluetooth RESET HIGH, 30ms-low 150ms-high turnON!!\n");
                msleep(150);
	} else {
		gpio_direction_output(bt_reset_gpio, 0);
...
#else
	bt_reset_gpio = GPIO_BT_RESET_N;
#endif
	rc = gpio_request(bt_reset_gpio, "bt_reset");

	if (rc) {
		dev_err(&pdev->dev, "GPIO req error no=%d \n", rc);
