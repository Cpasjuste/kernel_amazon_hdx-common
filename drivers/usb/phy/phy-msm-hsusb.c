...
struct msm_hsphy {
	...
	int			vdd_levels[4]; /* none, low, high */
	u32			lpm_flags;
	bool			suspended;
...
{
	int ret;
	pr_info("%s voting for vdd %d\n", __func__, phy->vdd_levels[high]);
	ret = regulator_set_voltage(phy->vdd, phy->vdd_levels[high],
				    phy->vdd_levels[2]);
	if (ret) {
	...
	dev_dbg(phy->phy.dev, "%s: min_vol:%d max_vol:%d\n", __func__,
		phy->vdd_levels[high], phy->vdd_levels[2]);
...
static int msm_hsphy_init(struct usb_phy *uphy)
{
	...
	if (phy->hsphy_init_seq) {
		int i;
		/* Set HSPHY override regs for every HSPHY port */
		for (i = 0; i < phy->num_ports; i++) {
			msm_usb_write_readback(phy->base,
				PARAMETER_OVERRIDE_X_REG(i), 0x03FFFFFF,
...
static int msm_hsphy_set_suspend(struct usb_phy *uphy, int suspend)
{
	...
	if (suspend) {
		...
		if (phy->lpm_flags & PHY_RETENTIONED || (host && (uphy->flags & PHY_PD_INSTALLED))) {
			if (!host && phy->ext_vbus_id) {
				msm_hsusb_ldo_enable(phy, 0);
				phy->lpm_flags |= PHY_PWR_COLLAPSED;
			}
			if (uphy->flags & PHY_HS_DEV_CONNECTED)
				msm_hsusb_config_vdd(phy, 3);
			else
...
static int msm_hsphy_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(dev->of_node, "qcom,dpdm-pd-installed"))
		phy->phy.flags |= PHY_PD_INSTALLED;
