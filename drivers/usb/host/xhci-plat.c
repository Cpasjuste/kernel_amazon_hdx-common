...
static int xhci_plat_bus_resume(struct usb_hcd *hcd)
{
	...
	if(xhci->resume_gpio){
		...
		gpio_direction_output(xhci->resume_gpio, 1);
	}
	ret = xhci_bus_resume(hcd);

	if(xhci->resume_gpio)
		gpio_direction_output(xhci->resume_gpio, 0);
...
static int xhci_plat_probe(struct platform_device *pdev)
{
	...
	*((struct xhci_hcd **) xhci->shared_hcd->hcd_priv) = xhci;
	...
	if (res) {
		xhci->resume_gpio = res->start;
		ret = gpio_request(xhci->resume_gpio,"USB_RESUME");
		if (ret < 0)
			dev_err(&pdev->dev, "Requesting resume GPIO failed = %d\n",ret);
		else {
			xhci->quirks |= XHCI_NO_SELECTIVE_SUSPEND;
			xhci->quirks |= XHCI_TR_DEQ_RESET_QUIRK;
		}
		dev_info(&pdev->dev,"Resume GPIO registered, gpio %d\n",(int)res->start);
	}
