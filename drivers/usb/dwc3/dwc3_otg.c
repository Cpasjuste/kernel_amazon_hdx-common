...
static int dwc3_otg_usbdev_notify(struct notifier_block *self,
			unsigned long action, void *priv)
{
	struct dwc3_otg *dotg = container_of(self, struct dwc3_otg, usbdev_nb);
	struct usb_device *udev = priv;
	char *disconnected[2] = { "USB_HOST=DISCONNECTED", NULL };
	char *connected[2] = { "USB_HOST=CONNECTED", NULL };
	char *configured[2] = { "USB_HOST=CONFIGURED", NULL };

	if (action == USB_BUS_ADD || action == USB_BUS_REMOVE)
		goto out;

	if (udev->bus != dotg->otg.host) {
		pr_info("%s: bus no equal\n", __func__);
		goto out;
	}

	switch (action) {
	case USB_DEVICE_ADD:
		kobject_uevent_env(&dotg->otg.phy->dev->kobj, KOBJ_CHANGE,
								connected);
		break;
	...
	case USB_DEVICE_REMOVE:
		pr_info("%s: sending DISCONNECTED\n", __func__);
		kobject_uevent_env(&dotg->otg.phy->dev->kobj, KOBJ_CHANGE,
								disconnected);
		break;
	default:
...
static int dwc3_otg_start_host(struct usb_otg *otg, int on)
{
	...
	if (on) {
		...
		dotg->usbdev_nb.notifier_call = dwc3_otg_usbdev_notify;
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_A_IDLE:
		...
		if (test_bit(ID, &dotg->inputs)) {
			...
			if ((ret < 0) && (dotg->vbus_retry_count < 3)) {
				/*
