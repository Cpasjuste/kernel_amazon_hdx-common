...
static int32_t afe_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		if (!this_afe.calib_data.status) {
			atomic_set(&this_afe.state, 0);
#ifndef CONFIG_SND_SOC_MAX98925
		...
		union afe_dsm_spkr_prot_config *prot_config)
{
	...
	pr_info("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$%s  port:0x%x, onoff:%d\n", __func__, port, prot_config->mode_dsm_proc_cfg.cntrlParams.dsmenabled);
	memset(&config, 0 , sizeof(config));
	if (!prot_config) {
		pr_err("%s Invalid params\n", __func__);
		goto fail_cmd;
...
#ifdef DSM_READ_FROM_FILE
		{
		uint32_t filter_set = DSM_ID_FILTER_SET_AFE_CNTRLS_FROM_BIN_FILE;//DSM_ID_FILTER_SET_AFE_CNTRLS_FROM_TXT_FILE;
...
int afe_dsm_spk_prot_get_calib_data(struct afe_dsm_spkr_prot_get_vi_calib *calib_resp)
{
	...
	calib_resp->get_param.payload_size = sizeof(*calib_resp)
		- sizeof(calib_resp->get_param);
	pr_debug("[MAXIM] %s: payload_size = %d\n", __func__, calib_resp->get_param.payload_size);
	calib_resp->get_param.port_id = q6audio_get_port_id(port);
	calib_resp->pdata.module_id = AFE_PARAM_ID_ENABLE_DSM_RX;//AFE_MODULE_FB_SPKR_PROT_VI_PROC;
	calib_resp->pdata.param_id = AFE_PARAM_ID_CALIB_RES_CFG;
	calib_resp->pdata.param_size = sizeof(calib_resp->res_cfg);
	pr_debug("[MAXIM] %s: param_size = %d\n", __func__, calib_resp->pdata.param_size);
	atomic_set(&this_afe.state, 1);
	ret = apr_send_pkt(this_afe.apr, (uint32_t *)calib_resp);
	if (ret < 0) {
...
#ifdef CONFIG_SND_SOC_MAX98925//X // AFE
int afe_dsm_spk_prot_feed_back_cfg(int src_port, DSM_Params_Ext_t *dsm_set_config)
{
