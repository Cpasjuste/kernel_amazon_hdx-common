...
#define GPIO_QUAT_MI2S_SCK    79
#define GPIO_QUAT_MI2S_WS     80
#define GPIO_QUAT_MI2S_DATA0  81
...
static void msm8974_mi2s_shutdown(struct snd_pcm_substream *substream)
{
        ...
        if (atomic_dec_return(&quat_mi2s_clk.mi2s_rsc_ref) == 0) {
                ...
                if (quat_mi2s_clk.bit_clk) {
                        clk_disable_unprepare(quat_mi2s_clk.bit_clk);
                        clk_put(quat_mi2s_clk.bit_clk);
                        quat_mi2s_clk.bit_clk = NULL;
                }

                if (quat_mi2s_clk.osr_clk) {
                        clk_disable_unprepare(quat_mi2s_clk.osr_clk);
                        clk_put(quat_mi2s_clk.osr_clk);
                        quat_mi2s_clk.osr_clk = NULL;
                }
                if (quat_mi2s_clk.core_clk) {
                        clk_put(quat_mi2s_clk.core_clk);
                        quat_mi2s_clk.core_clk = NULL;
                }
...
static int msm8974_mi2s_startup(struct snd_pcm_substream *substream)
{
        ...
        if (atomic_inc_return(&quat_mi2s_clk.mi2s_rsc_ref) == 1) {
                ...
                pr_debug("%s: clk device name %s\n", __func__,dev_name(cpu_dai->dev));

               /* Set up core clock. */
                quat_mi2s_clk.core_clk = clk_get(cpu_dai->dev, "core_clk");
                if (IS_ERR(quat_mi2s_clk.core_clk)) {
                        dev_err(cpu_dai->dev, "%s: Failed to Core clk %ld\n"
                               "CPU dai name %s\n", __func__,
                               PTR_ERR(quat_mi2s_clk.core_clk),
                               cpu_dai->dev->driver->name);
                        return -ENODEV ;
                }
                /* osr clock */
                quat_mi2s_clk.osr_clk = clk_get(cpu_dai->dev, "osr_clk");
                if (IS_ERR(quat_mi2s_clk.osr_clk)) {
                        dev_err(cpu_dai->dev, "%s: Failed to request OSR %ld\n"
                               "CPU dai name %s\n", __func__,
                               PTR_ERR(quat_mi2s_clk.osr_clk),
                               cpu_dai->dev->driver->name);
                        clk_put(quat_mi2s_clk.core_clk);
                        return -ENODEV ;
                }
                clk_set_rate(quat_mi2s_clk.core_clk, 12288000);
                 ret = clk_prepare_enable(quat_mi2s_clk.osr_clk);
                if (ret != 0) {
                        dev_err(cpu_dai->dev, "Fail to enable cdc_osr_clk\n");
                        goto exit_osrclk_err;
                }
                /* ibit clock */
                quat_mi2s_clk.bit_clk = clk_get(cpu_dai->dev, "ibit_clk");
                if (IS_ERR(quat_mi2s_clk.bit_clk)) {
                        dev_err(cpu_dai->dev, "%s: Failed to request Bit %ld\n"
                               "CPU dai name %s\n", __func__,
                               PTR_ERR(quat_mi2s_clk.bit_clk),
                               cpu_dai->dev->driver->name);
                        clk_put(quat_mi2s_clk.core_clk);
                        clk_put(quat_mi2s_clk.osr_clk);
                        return -ENODEV ;
                ...
                clk_set_rate(quat_mi2s_clk.core_clk, 12288000);
             //   clk_set_rate(quat_mi2s_clk.osr_clk, 12288000);
                clk_set_rate(quat_mi2s_clk.bit_clk, 7);
               ...
               /* ret = clk_prepare_enable(quat_mi2s_clk.osr_clk);
                ...
                ret = clk_prepare_enable(quat_mi2s_clk.bit_clk);
                if (ret != 0) {
                        dev_err(cpu_dai->dev, "Fail to enable cdc_bit_clk\n");
                        goto exit_bclk_err;
                }
...
#endif
	dev_err(&pdev->dev,"%s: success exit\n",__func__);
	return 0;
...
err:
	...
	dev_err(&pdev->dev,"%s: error(%d) exit\n",__func__, ret);
	return ret;
}
